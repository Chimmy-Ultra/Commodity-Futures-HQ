...
async function runWorkflowDAG({ nodes, edges, commodity, question, onEvent }) {
  ...
  for (var li = 0; li < levels.length; li++) {
    ...
    var promises = levelNodes.map(function (nodeId) {
      ...
      // Report output — assemble inbound results into a report
      if (node.type === 'report_output') {
        if (!context.length) return Promise.resolve();
        var report;
        if (context.length === 1) {
          report = context[0].response;
        } else {
          report = context.map(function (r) {
            return '## ' + r.name + '\n\n' + r.response;
          }).join('\n\n---\n\n');
        }
        onEvent('synthesis_start', { nodeId: nodeId });
        onEvent('synthesis', { nodeId: nodeId, report: report });
        results[nodeId] = { name: 'Report Output', response: report };
        return Promise.resolve();
      }
      ...
      // Agent node — supports any character that has a system prompt
      if (PROMPTS[node.type]) {
        var agentId = node.type;
        var name = AGENT_NAMES[agentId] || agentId;
        var subQ = SUB_QUESTIONS[agentId]
          ? SUB_QUESTIONS[agentId](commodity || 'this commodity')
          : (commodity
              ? 'Analyze the current situation for ' + commodity + '. Share your perspective based on your expertise.'
              : (question || 'Share your expert perspective on this topic.'));
