  ...
  return new Promise(function (resolve, reject) {
    ...
    proc.on('close', function (code) {
      ...
      try {
        ...
        if (parsed.usage || parsed.total_cost_usd !== undefined) {
          var u = parsed.usage || {};
          console.log('[tokens] input=' + (u.input_tokens || '?') + ' output=' + (u.output_tokens || '?') + ' cost=$' + (parsed.total_cost_usd !== undefined ? parsed.total_cost_usd.toFixed(4) : '?'));
        }
